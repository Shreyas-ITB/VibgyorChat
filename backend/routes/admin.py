...
from config import ADMIN_DASHBOARD_USERNAME, ADMIN_DASHBOARD_PASSWORD, ALLOWED_EMPLOYEE_DOMAINS_LIST
...
class AdminBypassLoginRequest(BaseModel):
    email_base64: str  # Base64 encoded email
    admin_username_hash: str  # SHA256 hash
    admin_password_hash: str  # SHA256 hash
...
async def create_employee(
    ...
    if not is_email_domain_allowed(payload.email):
        allowed_domains_str = ", ".join(ALLOWED_EMPLOYEE_DOMAINS_LIST)
        raise HTTPException(
            status_code=403,
            detail=f"Email domain not allowed for employees. Allowed domains: {allowed_domains_str}"
        )
...
async def admin_bypass_login(
    payload: AdminBypassLoginRequest,
    db=Depends(get_database)
):
    """
    Admin bypass login - Skip OTP for existing users (Admin panel only)
    
    This endpoint allows admins to generate tokens for existing users
    without going through the OTP process. Only works for users with
    ...
    # -------------------------
    try:
        email_bytes = base64.b64decode(payload.email_base64)
        email = email_bytes.decode('utf-8').strip().lower()
    except Exception as e:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid base64 email format: {str(e)}"
        )
    ...
    if not is_email_domain_allowed(email):
        ...
        raise HTTPException(
            status_code=403,
            detail=f"Email domain not allowed for bypass login. Allowed domains: {allowed_domains_str}"
        )
    ...
    # -------------------------
    is_admin = user.get("employ_id") is not None
    
    # -------------------------
    # GENERATE TOKENS
    # -------------------------
    payload_data = {"uid": email}
    ...
    # -------------------------
    # RESPONSE (SAME FORMAT AS OTHER LOGIN ENDPOINTS)
    # -------------------------
    return {
        "success": True,
        "complete": True,  # Assume admin users have complete profiles
        "is_admin": is_admin,
...
async def get_allowed_employee_domains():
    """
    Get the list of allowed email domains for employee accounts
    """
    
    return {
        "success": True,
        "allowed_domains": ALLOWED_EMPLOYEE_DOMAINS_LIST,
