...
async def google_callback(request: Request, state: str = Query(None), db=Depends(get_database)):
    ...
    # -------------------------
    # CHECK EMPLOYEE DOMAIN RESTRICTION
    # -------------------------
    from config import ALLOWED_EMPLOYEE_DOMAINS_LIST
    
    def is_email_domain_allowed(email: str) -> bool:
        """Check if the email domain is in the allowed employee domains list"""
        if not ALLOWED_EMPLOYEE_DOMAINS_LIST:
            return False
        
        for domain in ALLOWED_EMPLOYEE_DOMAINS_LIST:
            if email.lower().endswith(domain.lower()):
                return True
        return False
    
    # Check if email is from an employee domain
    if is_email_domain_allowed(email):
        # Email is from employee domain - check if user exists as employee
        existing_user = await users_collection.find_one({"email": email})
        
        if not existing_user:
            # User doesn't exist in database but has employee domain
            # Extract domain for error message
            domain = None
            for allowed_domain in ALLOWED_EMPLOYEE_DOMAINS_LIST:
                if email.lower().endswith(allowed_domain.lower()):
                    domain = allowed_domain
                    break
            
            raise HTTPException(
                status_code=403,
                detail=f"Employee Account Required\n\nIt looks like you have to be an employee ({domain}), but your account doesn't exist in the admin panel. Please add yourself as an employee to continue logging in."
            )
        
        # Check if user has employ_id (is actually an employee)
        if not existing_user.get("employ_id"):
            # User exists but doesn't have employ_id - they need to be added as employee
            domain = None
            for allowed_domain in ALLOWED_EMPLOYEE_DOMAINS_LIST:
                if email.lower().endswith(allowed_domain.lower()):
                    domain = allowed_domain
...
async def email_login(payload: dict, db=Depends(get_database)):
    email_or_username = payload.get("email")  # Can be either email or username
    otp = payload.get("otp")
    ...
    if is_email(email_or_username):
        ...
        lookup_user = await users.find_one({"email": email})
    else:
        # Input is a username - look up the email
        username = email_or_username.strip()
        lookup_user = await users.find_one({"username": username})
        
        if not lookup_user:
            raise HTTPException(
                status_code=404, 
                detail="Username not found. Please check your username or use your email address."
            )
    ...
    if not otp:
        ...
        return {
            "success": True,
            "otp_sent": True,
            "email_used": email,  # Show which email the OTP was sent to
            "login_method": "email" if is_email(email_or_username) else "username"
        }
    ...
    # Re-fetch user data (in case it was updated between OTP request and verification)
    ...
    # ----------------------------------
    # NEW USER (should be rare since we looked up by username)
    # ----------------------------------
    if not existing_user:
        user = UserCreate(
            email=email,
