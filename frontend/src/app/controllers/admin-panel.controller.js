...
angular.module('vibgyorChat').controller('AdminPanelController', ['$scope', '$location', '$http', '$timeout', function($scope, $location, $http, $timeout) {
  ...
  if (!ADMIN_USERNAME || !ADMIN_PASSWORD) {
    ...
    $scope.error = 'Admin credentials not configured. Please check your environment variables.';
    return;
  }
  ...
  $scope.testAdminCredentials = function() {
    console.log('🧪 Testing admin credentials...');
    const testUrl = `${API_BASE_URL}/admin/statistics?admin_username_hash=${usernameHash}&admin_password_hash=${passwordHash}`;
    
    $http.get(testUrl)
      .then(function(response) {
        console.log('✅ Admin credentials test successful:', response.data);
      })
      .catch(function(error) {
        console.error('❌ Admin credentials test failed:', error);
        if (error.status === 401 || error.status === 403) {
          console.error('❌ Admin credentials are invalid or API is not accepting them');
        }
      });
  };
  ...
  $scope.allowedDomains = []; // Store allowed employee domains
  ...
  // Fetch allowed employee domains
  $scope.fetchAllowedDomains = function() {
    console.log('🌐 Fetching allowed employee domains...');
    
    const url = `${API_BASE_URL}/admin/allowed-domains?admin_username_hash=${usernameHash}&admin_password_hash=${passwordHash}`;
    
    $http.get(url)
      .then(function(response) {
        console.log('✅ Allowed domains response:', response.data);
        
        if (response.data.success && response.data.allowed_domains) {
          $scope.allowedDomains = response.data.allowed_domains;
          console.log('📋 Allowed domains:', $scope.allowedDomains);
        } else {
  ...
  $scope.validateEmailDomain = function(email) {
    if (!email || $scope.allowedDomains.length === 0) {
      return true; // Skip validation if no domains configured
    }
    
    const emailLower = email.toLowerCase();
    
    // Check if email ends with any of the allowed domains
    for (let domain of $scope.allowedDomains) {
  ...
  // Get allowed domains as a readable string
  $scope.getAllowedDomainsString = function() {
    if ($scope.allowedDomains.length === 0) {
      return 'any domain';
    }
    return $scope.allowedDomains.join(', ');
  };
  ...
  $scope.checkEmployeeUsername = function() {
    ...
    // For admin panel, include admin credentials in the query
    const url = `${API_BASE_URL}/auth/login/checkusername?q=${encodeURIComponent($scope.employeeForm.username)}&admin_username_hash=${usernameHash}&admin_password_hash=${passwordHash}`;
    
    console.log('🔍 Checking username availability (admin mode):', url);
    ...
    $http.get(url)
      ...
      .catch(function(error) {
        ...
        if (error.status === 401) {
          $scope.usernameError = 'Unauthorized: Invalid admin credentials for username check';
        } else if (error.status === 403) {
          $scope.usernameError = 'Forbidden: Admin access required for username check';
        } else {
          $scope.usernameError = 'Error checking username availability';
        }
  ...
  $scope.saveEmployee = function() {
    ...
    console.log('🔐 Using admin credentials - Username hash:', usernameHash);
    console.log('🔐 Using admin credentials - Password hash:', passwordHash);
    ...
    // Validate email domain for add mode
    if ($scope.employeeModalMode === 'add') {
      if (!$scope.validateEmailDomain($scope.employeeForm.email)) {
        $scope.employeeFormError = `Email domain not allowed for employees. Allowed domains: ${$scope.getAllowedDomainsString()}`;
        return;
      }
    }
    ...
    httpMethod(url, requestBody)
      ...
      .catch(function(error) {
        ...
        if (error && error.data && error.data.detail) {
          // Check if it's a domain validation error from backend
          if (error.data.detail.includes('Email domain not allowed')) {
            $scope.employeeFormError = error.data.detail;
          } else if (error.data.detail.includes('Invalid admin credentials')) {
            $scope.employeeFormError = 'Invalid admin credentials. Please check your admin login.';
            console.error('❌ Admin credentials invalid - redirecting to admin login');
          ...
          if (error.status === 401) {
            $scope.employeeFormError = 'Unauthorized: Invalid admin credentials. Please check your admin login.';
          } else if (error.status === 403) {
            $scope.employeeFormError = 'Forbidden: You do not have permission to perform this action.';
          } else if (error.status === 404) {
            $scope.employeeFormError = 'Employee endpoint not found. Please check the API configuration.';
          } else if (error.status === 0) {
            $scope.employeeFormError = 'Cannot connect to server. Please check your internet connection.';
          } else {
            $scope.employeeFormError = `Server error: ${error.status} - ${error.statusText || 'Unknown error'}`;
          }
        } else {
          $scope.employeeFormError = 'Unknown error occurred while saving employee';
        }
