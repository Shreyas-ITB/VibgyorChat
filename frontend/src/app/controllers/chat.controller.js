...
angular.module('vibgyorChat').controller('ChatController', ['$scope', '$rootScope', '$timeout', '$q', 'AuthService', 'UserService', 'ChatService', 'SocketService', 'ThemeService', 'ToastService', 'FileService', function($scope, $rootScope, $timeout, $q, AuthService, UserService, ChatService, SocketService, ThemeService, ToastService, FileService) {
  ...
  $scope.filterContacts = function(filterType) {
    ...
    switch(filterType) {
      case 'all':
        // Exclude archived contacts from main list, sort with pinned ones first
        $scope.filteredContacts = $scope.contacts.filter(c => !c.archived).sort(function(a, b) {
          if (a.pinned && !b.pinned) return -1;
          if (!a.pinned && b.pinned) return 1;
          return 0;
        });
        // Separate archived contacts
        $scope.archivedContacts = $scope.contacts.filter(c => c.archived);
        break;
      case 'muted':
        $scope.filteredContacts = $scope.contacts.filter(c => c.muted && !c.archived);
        $scope.archivedContacts = $scope.contacts.filter(c => c.archived);
        break;
      case 'favorites':
        $scope.filteredContacts = $scope.contacts.filter(c => c.favorited && !c.archived);
        $scope.archivedContacts = $scope.contacts.filter(c => c.archived);
        break;
      default:
  ...
  $scope.filterGroups = function(filterType) {
    ...
    switch(filterType) {
      case 'all':
        // Exclude archived groups from main list, sort with pinned ones first
        ...
        break;
      case 'muted':
        $scope.filteredGroups = $scope.groups.filter(g => g.muted && !g.archived);
        break;
      case 'favorites':
        $scope.filteredGroups = $scope.groups.filter(g => g.is_favorited && !g.archived);
        break;
      default:
        $scope.filteredGroups = $scope.groups.filter(g => !g.archived);
    }
  };
  ...
  $scope.joinGroupById = function(groupId) {
    ...
    ChatService.joinGroup(groupId).then(function(response) {
      if (response.already_member) {
        ToastService.info('You are already a member of this group');
      } else {
        ToastService.success(`Successfully joined ${response.group_name || 'the group'}`);
      }
      
      // Reload groups to show the newly joined group
      return $scope.loadGroups();
    }).then(function() {
      // After groups are reloaded, try to select the newly joined group
      const allGroups = ($scope.groups || []).concat($scope.archivedGroups || []);
      ...
      if (error.data && error.data.message) {
        ...
        ToastService.error('Failed to join group. Please try again.');
      }
  ...
  $scope.selectFile = function() {
    ...
    input.onchange = function(event) {
      ...
      if (files.length > 0) {
        ...
        $timeout(function() {
          files.forEach(function(file) {
            // Validate file size (10MB limit for files)
  ...
  $scope.stageFileForPreview = function(file, fileType) {
    ...
    // Convert to base64 for preview and later sending
    FileService.fileToBase64(file).then(function(base64Data) {
      $timeout(function() {
        preview.base64Data = base64Data;
        
        // Create preview URL for images
        if (fileType === 'image') {
          preview.previewUrl = 'data:image/' + file.name.split('.').pop() + ';base64,' + base64Data;
        }
      });
    }).catch(function(error) {
      $timeout(function() {
        console.error('Failed to process file:', error);
        ToastService.error('Failed to process file: ' + file.name);
        // Remove failed preview from array
        const index = $scope.filePreviews.indexOf(preview);
        if (index > -1) {
  ...
  $scope.sendStagedFiles = function() {
    ...
    const allProcessed = $scope.filePreviews.every(function(preview) {
      return preview.base64Data !== null;
    });

    if (!allProcessed) {
      ToastService.error('Please wait for all files to be processed');
      return;
    }

    // Validate file sizes before sending
    for (let i = 0; i < $scope.filePreviews.length; i++) {
      const preview = $scope.filePreviews[i];
      const maxSize = preview.fileType === 'image' ? 5 : 10;
      
      if (!$scope.validateFileSize(preview.file, maxSize, preview.fileType)) {
    ...
    try {
      // Send each file
      $scope.filePreviews.forEach(function(preview) {
      ...
      const fileCount = $scope.filePreviews.length;
      ToastService.success(fileCount + ' file(s) sent successfully');
      
      // Clear all previews
      $scope.filePreviews = [];
      
      // Refresh messages after a short delay to ensure backend has processed the upload
      $timeout(function() {
        if (ChatService.currentConversation) {
          console.log('Refreshing messages after file upload');
          ChatService.loadMessages(ChatService.currentConversation.id).then(function() {
            $scope.messages = ChatService.messages;
            $scope.scrollToBottomUniversal(true);
            console.log('Messages refreshed, count:', $scope.messages.length);
          }).catch(function(error) {
      ...
      ToastService.error('Failed to send files');
      $scope.uploadingFile = false;
    }
  };
  ...
  $scope.validateFileSize = function(file, maxSizeMB, fileType) {
    ...
    if (file.size > maxSizeBytes) {
      ...
      const fileTypeText = fileType === 'image' ? 'image' : 'file';
  ...
  $scope.loadGroups = function() {
    ...
    return $q.all([groupsPromise, groupDataPromise]).then(function(results) {
      ...
      $scope.archivedGroups = allGroups.filter(function(group) {
        return group.archived;
      });

      // Filter out archived groups from main list
      $scope.groups = allGroups.filter(function(group) {
        return !group.archived;
      });
  ...
  $scope.showGroupSettings = function() {
    ...
    $scope.groupSettingsModal = {
      ...
      inviteLink: window.APP_CONFIG.FRONTEND_URL + '/invite/' + group.id // Generate invite link
    };
  ...
  // Theme functionality - disabled for now (dark mode only)
