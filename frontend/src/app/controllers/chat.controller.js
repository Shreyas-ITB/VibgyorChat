...
angular.module('vibgyorChat').controller('ChatController', ['$scope', '$rootScope', '$timeout', '$q', '$sce', 'AuthService', 'UserService', 'ChatService', 'SocketService', 'ThemeService', 'ToastService', 'FileService', 'InviteService', 'JoinRequestService', 'VersionService', function($scope, $rootScope, $timeout, $q, $sce, AuthService, UserService, ChatService, SocketService, ThemeService, ToastService, FileService, InviteService, JoinRequestService, VersionService) {
  ...
  // Helper function for smooth scrolling within message containers
  $scope.scrollToMessageElement = function(messageElement, callback) {
    if (!messageElement) return;
    
    // Find the messages container
    var messagesContainer = document.getElementById('messages-container') || 
                           document.getElementById('mobile-messages-container') ||
                           messageElement.closest('.overflow-y-auto, .messages-list, .mobile-messages-list');
    ...
    if (messagesContainer) {
      ...
      // Calculate the target scroll position to center the message in the container
      var targetScrollTop = containerScrollTop + (messageRect.top - containerRect.top) - (containerRect.height / 2) + (messageRect.height / 2);
      
      // Ensure we don't scroll beyond the container bounds
      var maxScrollTop = messagesContainer.scrollHeight - messagesContainer.clientHeight;
      ...
      // Fallback: use scrollIntoView but with 'nearest' to minimize page jumping
  ...
  $scope.jumpToMessage = function(message) {
    ...
    // Scroll to message within the messages container only
    $timeout(function() {
      var messageElement = document.querySelector('[data-message-id="' + (message.id || message._id) + '"]');
      if (messageElement) {
        $scope.scrollToMessageElement(messageElement, function() {
  ...
  $scope.insertEmoji = function(emoji, event) {
    if (!$scope.messageInput) {
      $scope.messageInput = '';
    }
    
    // Find the active textarea (desktop or mobile)
    const textarea = document.querySelector('textarea[ng-model="messageInput"]') || 
                    document.querySelector('.mobile-input-field');
    
    if (textarea) {
      const cursorPos = textarea.selectionStart || $scope.messageInput.length;
      ...
      // Try to focus any available input after a short delay
      $timeout(function() {
    ...
    // Check if shift key is held - if not, close the picker
  ...
  $scope.init = function() {
    ...
    const inviteMatch = currentPath.match(/\/invite\/([A-Za-z0-9_-]+)/);
    if (inviteMatch) {
      const inviteToken = inviteMatch[1];
      
      // Validate the invite token before processing
      const validation = InviteService.validateInviteLink(inviteToken);
      
      if (!validation.valid) {
        // Invalid, expired, or revoked invite
        console.error('❌ Invalid invite link:', validation.reason, validation.message);
      ...
      // Valid invite - store for processing after initialization
      $scope.pendingInviteToken = inviteToken;
      $scope.pendingInviteGroupId = validation.groupId;
      
      console.log('✅ Valid invite token found:', inviteToken, 'for group:', validation.groupId);
      
      // Store invite information for cross-session persistence (in case user needs to log in)
      localStorage.setItem('pendingGroupInvite', validation.groupId);
  ...
  // Enhanced pending group invite handling with backend validation
  $scope.handlePendingGroupInvite = function() {
    ...
    let pendingToken = $scope.pendingInviteToken || localStorage.getItem('pendingInviteToken');
    
    if (pendingGroupId) {
      // If we have a token, validate it rigorously with backend before proceeding
      if (pendingToken) {
        console.log('🔍 Validating pending invite token with backend...');
        
        InviteService.validateInviteLink(pendingToken).then(function(validation) {
          if (!validation.valid) {
            console.error('❌ Invite token is no longer valid:', validation.reason, validation.message);
            
            // Show specific error messages based on reason
            if (validation.reason === 'revoked') {
              ToastService.error('This invite link was revoked by the group admin. Please request a new link.');
            } else if (validation.reason === 'expired') {
              ToastService.error('This invite link has expired. Please request a new link.');
            } else {
              ToastService.error(validation.message);
            }
            
            // Clear all pending invite data
            localStorage.removeItem('pendingGroupInvite');
            localStorage.removeItem('pendingInviteToken');
            $scope.pendingInviteGroupId = null;
            $scope.pendingInviteToken = null;
            return;
          }
          
          console.log('✅ Invite token validated successfully with backend, proceeding with join');
          
          // Proceed with group join
          $scope.proceedWithGroupJoin(pendingGroupId, pendingToken);
        }).catch(function(error) {
          console.error('❌ Failed to validate invite token with backend:', error);
          ToastService.error('Failed to validate invite link. Please try again or request a new link.');
          
          // Clear pending invite data on validation failure
          localStorage.removeItem('pendingGroupInvite');
          localStorage.removeItem('pendingInviteToken');
          $scope.pendingInviteGroupId = null;
          $scope.pendingInviteToken = null;
        ...
        console.warn('⚠️ No invite token found for pending group invite');
        $scope.proceedWithGroupJoin(pendingGroupId, null);
      }
    }
  };

  // Proceed with group join after validation
  $scope.proceedWithGroupJoin = function(pendingGroupId, pendingToken) {
  ...
  $scope.loadPendingRequestGroups = function() {
    ...
    JoinRequestService.getMyPendingRequests().then(function(response) {
      ...
      console.log('🔍 Sample pending request group structure:', $scope.pendingRequestGroups.length > 0 ? $scope.pendingRequestGroups[0] : 'No pending requests');
  ...
  $scope.cancelJoinRequest = function(conversationId) {
    ...
    console.log('🔍 Available group properties in pending requests:', $scope.pendingRequestGroups.length > 0 ? Object.keys($scope.pendingRequestGroups[0]) : 'No pending requests');
    
    if (!conversationId) {
      console.error('❌ No conversation ID provided for cancel request');
      ToastService.error('Unable to cancel request - missing group ID');
      return;
    }
    ...
    JoinRequestService.cancelRequest(conversationId).then(function(response) {
      ...
      ToastService.success(`Cancelled request to join ${response.group_name || 'group'}`);
  ...
  // Enhanced group joining with backend invite validation
  ...
  $scope.joinGroupById = function(groupId) {
    ...
    // Enhanced validation: Check if this join attempt is from a valid invite
    let inviteToken = $scope.pendingInviteToken || localStorage.getItem('pendingInviteToken');
    
    if (inviteToken) {
      console.log('🔍 Final validation of invite token with backend before joining...');
      
      // Validate the invite token one more time with backend before joining
      InviteService.validateInviteLink(inviteToken).then(function(validation) {
        if (!validation.valid) {
          console.log('🚫 Blocked join attempt for invalid/revoked invite:', {
            token: inviteToken.substring(0, 20) + '...',
          ...
          if (validation.reason === 'revoked') {
            ToastService.error('This invite link was just revoked! Please request a new link from the group admin.');
          } else if (validation.reason === 'expired') {
            ToastService.error('This invite link expired! Please request a new link from the group admin.');
          } else {
            ToastService.error(validation.message);
          }
          ...
          // Clean up all invite-related data
          localStorage.removeItem('pendingGroupInvite');
          localStorage.removeItem('pendingInviteToken');
          $scope.pendingInviteToken = null;
          $scope.pendingInviteGroupId = null;
        ...
        console.log('✅ Invite token validated successfully with backend, proceeding with join request');
        $scope.sendJoinRequest(groupId, inviteToken);
      }).catch(function(error) {
        console.error('❌ Failed to validate invite token with backend before join:', error);
        ToastService.error('Failed to validate invite link. Please try again.');
      ...
      // No invite token, proceed with regular join
      $scope.sendJoinRequest(groupId, null);
    }
  };

  // Send join request to backend
  $scope.sendJoinRequest = function(groupId, inviteToken) {
    console.log('✅ Sending join request for group:', groupId, inviteToken ? 'with invite token' : 'without invite token');
  ...
  $scope.selectContact = function(contact) {
    ...
    // Clear pin notification messages when switching conversations
    $scope.pinNotificationMessages = [];
  ...
  $scope.togglePinMessage = function(message) {
    if (!message) {
      console.error('No message provided to togglePinMessage');
      return;
    }
    
    const messageId = message.id || message._id;
    if (!messageId) {
      console.error('Message has no valid ID:', message);
      ToastService.error('Cannot pin message: Invalid message ID');
      return;
    }
    ...
    try {
      ...
      ToastService.error('Failed to toggle pin message');
    }
  };
  ...
  // Check if message can be edited or deleted based on timer
  $scope.canEditOrDeleteMessage = function(message) {
    if (!message) {
      return false;
    }
    
    // Check for timestamp or created_at field
    const messageTimestamp = message.timestamp || message.created_at;
    if (!messageTimestamp) {
      return false;
    }
    
    // Get the timer value from config (in minutes)
    const timerMinutes = window.APP_CONFIG.EDIT_DELETE_MESSAGE_TIMER || 15;
    const timerMs = timerMinutes * 60 * 1000; // Convert to milliseconds
    
    // Parse message timestamp
    let messageTime;
    if (typeof messageTimestamp === 'string') {
      messageTime = new Date(messageTimestamp).getTime();
    ...
    // Return true if message is within the allowed time window
    return timeDiff <= timerMs;
  };
  ...
  $scope.editMessage = function(message) {
    ...
    // Check if message is still within edit time limit
    if (!$scope.canEditOrDeleteMessage(message)) {
      const timerMinutes = window.APP_CONFIG.EDIT_DELETE_MESSAGE_TIMER || 15;
      ToastService.error(`Messages can only be edited within ${timerMinutes} minutes of sending`);
      return;
    }
  ...
  $scope.showDeleteMessageConfirmation = function(message) {
    ...
    // Check if message is still within delete time limit
    if (!$scope.canEditOrDeleteMessage(message)) {
      const timerMinutes = window.APP_CONFIG.EDIT_DELETE_MESSAGE_TIMER || 15;
      ToastService.error(`Messages can only be deleted within ${timerMinutes} minutes of sending`);
      return;
    }
  ...
  // Get the name of the person who pinned a message (not the message sender)
  $scope.getPinnerName = function(message) {
    if (!message) return 'Someone';
    
    // Check if we have pinned_by information
    if (message.pinned_by) {
      // Check if it's the current user
      if ($scope.currentUser && message.pinned_by === $scope.currentUser.email) {
        return 'You';
      }
      
      // Try to find the user in contacts first
      const contact = $scope.contacts.find(c => 
        c.email === message.pinned_by || 
        c.username === message.pinned_by
      );
      if (contact) {
        return contact.name || contact.username || contact.email;
      }
      
      // If not found in contacts, try groups members
      if ($scope.selectedContact && $scope.selectedContact.isGroup && $scope.selectedContact.members) {
        const member = $scope.selectedContact.members.find(m => 
          m.email === message.pinned_by || 
          m.username === message.pinned_by
        );
        if (member) {
      ...
      // Try to find in all groups members (in case it's from a different group context)
      if ($scope.groups) {
        for (let group of $scope.groups) {
          if (group.members) {
      ...
      // If it's an email, try to extract a readable name from it
      if (message.pinned_by.includes('@')) {
        const emailParts = message.pinned_by.split('@')[0];
        // Convert email prefix to a more readable format
        const readableName = emailParts.replace(/[._-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        return readableName;
      }
      
      // Return the pinned_by value directly as last resort
      return message.pinned_by;
    }
    
    // Fallback: if no pinned_by info, return 'Someone'
    return 'Someone';
  };

  // Async version to get pinner name with UserService lookup
  $scope.getPinnerNameAsync = function(pinnedBy) {
    ...
    // Check if it's the current user
    if ($scope.currentUser && pinnedBy === $scope.currentUser.email) {
      deferred.resolve('You');
      return deferred.promise;
    }
    ...
    if (contact) {
      deferred.resolve(contact.name || contact.username || contact.email);
      return deferred.promise;
    }
    ...
    if ($scope.selectedContact && $scope.selectedContact.isGroup && $scope.selectedContact.members) {
      ...
      if (member) {
        deferred.resolve(member.name || member.username || member.email);
        return deferred.promise;
      }
    }
    
    // Try to find in all groups members
    if ($scope.groups) {
      for (let group of $scope.groups) {
        if (group.members) {
          const member = group.members.find(m => 
    ...
    UserService.getUserInfo(pinnedBy)
      .then(function(userInfo) {
        const displayName = userInfo.name || userInfo.username || userInfo.email;
        deferred.resolve(displayName);
      })
      .catch(function(error) {
        console.log('Could not get user info for:', pinnedBy, error);
        // Fallback to making email more readable
        if (pinnedBy.includes('@')) {
          const emailParts = pinnedBy.split('@')[0];
          const readableName = emailParts.replace(/[._-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
          deferred.resolve(readableName);
        } else {
          deferred.resolve(pinnedBy);
        }
      });
  ...
  $scope.setSidebarView = function(view) {
    ...
    // Fetch version information when settings are opened
    if (view === 'settings') {
      $scope.fetchVersionInfo();
    }
  };
  ...
  $scope.fetchVersionInfo = function() {
    if (!$scope.versionInfo.loading && !$scope.versionInfo.error) {
      return; // Already fetched successfully
    }
    
    $scope.versionInfo.loading = true;
    $scope.versionInfo.error = false;
    
    VersionService.getVersionInfo().then(function(data) {
      $scope.versionInfo = {
        api_version: data.api_version || 'Unknown',
        frontend_version: data.frontend_version || 'Unknown',
        loading: false,
        error: false
      };
      console.log('✅ Version info loaded:', $scope.versionInfo);
    }).catch(function(error) {
  ...
  // Group Settings Modal with enhanced invite link settings and loading states
  $scope.groupSettingsModal = {
    ...
    addMemberEmail: '',
    saving: false,
    // Enhanced invite link settings with loading states
    hasActiveInviteLink: false,
    inviteLink: '',
    inviteLinkExpiresAt: null,
    inviteLinkExpiration: 4, // Default to 4 days
  ...
  $scope.showGroupSettings = function() {
    ...
    $scope.groupSettingsModal = {
      ...
      // Enhanced invite link settings
      inviteLink: '',
      inviteLinkExpiration: 4, // Default 4 days
      inviteLinkExpiresAt: null,
      hasActiveInviteLink: false
    };
    
    // Load existing invite link or prepare for new one
  ...
  // Enhanced invite link loading with backend integration
  $scope.loadInviteLink = function() {
    if (!$scope.groupSettingsModal.group) return;
    
    const groupId = $scope.groupSettingsModal.group.id;
    
    // Get active invite link from backend
    InviteService.getActiveInviteLink(groupId).then(function(activeLink) {
      if (activeLink) {
        // Active valid link exists
        ...
        console.log('✅ Loaded active invite link from backend:', activeLink.uuid.substring(0, 20) + '...');
      } else {
        // No active link
        ...
        console.log('ℹ️ No active invite link found in backend');
      }
    }).catch(function(error) {
      console.error('❌ Failed to load invite link from backend:', error);
      $scope.groupSettingsModal.hasActiveInviteLink = false;
      $scope.groupSettingsModal.inviteLink = '';
      $scope.groupSettingsModal.inviteLinkExpiresAt = null;
    });
  ...
  // Enhanced invite link generation with backend storage
  $scope.generateInviteLink = function() {
    ...
    if (!expirationDays || ![2, 4, 6].includes(expirationDays)) {
      ToastService.error('Please select a valid expiration period (2, 4, or 6 days)');
      return;
    }
    
    // Show loading state
    $scope.groupSettingsModal.generatingLink = true;
    
    // First revoke all existing links, then create new one
    InviteService.revokeAllGroupInvites(groupId).then(function(revokedCount) {
      if (revokedCount > 0) {
        console.log('🚫 Revoked', revokedCount, 'existing invite links');
      }
      
      // Create new invite link and store in backend
      return InviteService.createInviteLink(groupId, expirationDays);
      ...
      $scope.groupSettingsModal.generatingLink = false;
      
      console.log('✅ Generated and stored new invite link:', {
        token: inviteData.uuid.substring(0, 20) + '...',
        expiresAt: inviteData.expiresAt,
        expirationDays: expirationDays
      });
      
      ToastService.success(`New ${expirationDays}-day invite link generated and stored!`);
    }).catch(function(error) {
      console.error('❌ Failed to generate invite link:', error);
      $scope.groupSettingsModal.generatingLink = false;
      
      if (error.status === 403) {
        ToastService.error('You do not have permission to create invite links for this group.');
      } else if (error.status === 401) {
        ToastService.error('Please log in again to create invite links.');
      } else {
        ToastService.error('Failed to generate invite link. Please try again.');
      }
    });
  };
  ...
  // Enhanced invite link revocation with backend removal
  $scope.revokeInviteLink = function() {
    ...
    // Show loading state
    $scope.groupSettingsModal.revokingLink = true;
    
    // Revoke the link from backend
    InviteService.revokeInviteLink(uuid).then(function(success) {
      if (success) {
        // Update modal state immediately
        $scope.groupSettingsModal.hasActiveInviteLink = false;
        $scope.groupSettingsModal.inviteLink = '';
        $scope.groupSettingsModal.inviteLinkExpiresAt = null;
        $scope.groupSettingsModal.revokingLink = false;
        
        ToastService.success('Invite link revoked immediately! No one can use this link anymore.');
        console.log('✅ Invite link revoked from backend:', uuid.substring(0, 20) + '...');
      } else {
        $scope.groupSettingsModal.revokingLink = false;
      ...
      if (error.status === 403) {
        ToastService.error('You do not have permission to revoke invite links for this group.');
      } else if (error.status === 401) {
        ToastService.error('Please log in again to revoke invite links.');
      } else {
        ToastService.error('Failed to revoke invite link. Please try again.');
      }
    });
  };
  ...
  $scope.showContextMenu = function(event, contact) {
    ...
    const menuWidth = 200;
    const menuHeight = 250; // Approximate height based on number of menu items
    ...
    // If menu would go off bottom, position it above the cursor
    if (y + menuHeight > screenHeight - 10) {
      y = screenHeight - menuHeight - 10;
    }
    
    // Ensure minimum margins from top and left
    x = Math.max(10, x);
    y = Math.max(10, y);
  ...
  $scope.pinChat = function(contact) {
    if (contact.isGroup) {
      ...
      ChatService.pinGroup(contact.id).then(function(response) {
        ...
        // Reload groups to get updated state and re-sort
        $scope.loadGroups().then(function() {
          // Explicitly re-apply the current filter to ensure proper sorting
          $timeout(function() {
            if ($scope.sidebarView === 'groups') {
              $scope.filterGroups($scope.groupFilter);
            } else {
              $scope.filterContacts($scope.filter);
            }
      ...
      UserService.pinContact(contact.email).then(function(response) {
        ...
        if (response && typeof response.pinned !== 'undefined') {
          ...
          // Also update in the contacts array
          const contactInArray = $scope.contacts.find(c => c.email === contact.email);
          if (contactInArray) {
            contactInArray.pinned = response.pinned;
            contactInArray.is_pinned = response.pinned;
          }
        }
        // Reload contacts to get updated state and re-sort
        $scope.loadContacts().then(function() {
          // Explicitly re-apply the current filter to ensure proper sorting
          $timeout(function() {
            $scope.filterContacts($scope.filter);
  ...
  $scope.showSearchContactMenu = function(event, contact) {
    ...
    // Context menu dimensions (approximate)
    const menuWidth = 200;
    const menuHeight = 150; // Approximate height for search contact menu
    const screenWidth = window.innerWidth;
    const screenHeight = window.innerHeight;
  ...
  // Store pin notification messages to preserve them across reloads
  $scope.pinNotificationMessages = [];
  $scope.recentPinEvents = new Set(); // Track recent pin events to avoid conflicts
  ...
  $scope.$on('message:updated', function(event, data) {
    ...
    if (ChatService.currentConversation) {
      // Store current pin notification messages before reload
      const currentPinNotifications = $scope.messages.filter(msg => msg.type === 'pin_notification');
      console.log('📌 Storing pin notifications before reload:', currentPinNotifications.length);
      
      $timeout(function() {
        ChatService.loadMessages(ChatService.currentConversation.id).then(function() {
          $scope.messages = ChatService.messages;
          
          // Re-add pin notification messages after reload, but skip recent pin events
          currentPinNotifications.forEach(notification => {
            // Skip if this is a recent pin event (will be handled by message:pinned)
            if ($scope.recentPinEvents.has(notification.pinned_message_id)) {
              console.log('📌 Skipping restoration of recent pin notification:', notification.pinned_message_id);
              return;
            }
            
            // Check if the pinned message still exists and is still pinned
            const originalMessage = $scope.messages.find(msg => 
              (msg.id || msg._id) === notification.pinned_message_id && (msg.pinned || msg.is_pinned)
            );
            
            if (originalMessage) {
              // Add the notification back to the messages
              $scope.messages.push(notification);
              console.log('📌 Restored pin notification:', notification.content);
            } else {
              console.log('📌 Pin notification not restored - original message not found or not pinned');
            }
          });
          
          // Sort messages by created_at to maintain chronological order
          $scope.messages.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
          
          $scope.updatePinnedMessages(); // Update pinned messages list
          console.log('Messages reloaded after update, pin notifications preserved');
        });
      }, 100);
    }
  });
  ...
  $scope.$on('message:pinned', function(event, data) {
    ...
    if (data.pinned) {
      // Track this as a recent pin event to avoid conflicts with message:updated
      $scope.recentPinEvents.add(data.message_id);
      
      // Clear the tracking after a short delay
      $timeout(function() {
        $scope.recentPinEvents.delete(data.message_id);
      }, 2000);
      
      // Get the name of the person who pinned it using async lookup
      $scope.getPinnerNameAsync(data.pinned_by)
        .then(function(pinnerName) {
          // Find the original message that was pinned
          const originalMessage = $scope.messages.find(msg => (msg.id || msg._id) === data.message_id);
          
          if (originalMessage) {
            // Remove any existing pin notification for this message first
            $scope.messages = $scope.messages.filter(msg => 
              !(msg.type === 'pin_notification' && msg.pinned_message_id === data.message_id)
            );
            
            // Create a system notification message that appears in the chat
            const notificationMessage = {
              id: 'pin_notification_' + data.message_id + '_' + Date.now(),
              type: 'pin_notification',
              content: `${pinnerName} pinned a message to this chat`,
              created_at: data.pinned_at || new Date().toISOString(),
              sender: 'system',
              sender_name: 'System',
              is_system_message: true,
              pinned_message_id: data.message_id,
              pinned_by: data.pinned_by,
              original_message: originalMessage
            };
            
            // Add the notification message to the chat
            $scope.messages.push(notificationMessage);
            
            // Sort messages by created_at to maintain chronological order
            $scope.messages.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
            
            console.log(`✅ Added pin notification message to chat: ${pinnerName} pinned a message`);
            console.log('📌 Pin notification message details:', notificationMessage);
            
            // Apply scope changes
            $scope.$apply();
            
            // Scroll to bottom to show the new notification
            $timeout(function() {
        ...
        .catch(function(error) {
          console.error('Failed to get pinner name:', error);
          // Fallback to original logic
          let pinnerName = 'Someone';
          if ($scope.currentUser && data.pinned_by === $scope.currentUser.email) {
            pinnerName = 'You';
          } else if (data.pinned_by) {
            if (data.pinned_by.includes('@')) {
              const emailParts = data.pinned_by.split('@')[0];
              pinnerName = emailParts.replace(/[._-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            } else {
              pinnerName = data.pinned_by;
            }
          }
          
          // Create notification with fallback name
          const originalMessage = $scope.messages.find(msg => (msg.id || msg._id) === data.message_id);
          if (originalMessage) {
            $scope.messages = $scope.messages.filter(msg => 
              !(msg.type === 'pin_notification' && msg.pinned_message_id === data.message_id)
      ...
      // Handle unpinning - remove the notification message
      let unpinnerName = 'Someone';
      
      // Check if pinned_by is the current user
      if ($scope.currentUser && data.pinned_by === $scope.currentUser.email) {
        unpinnerName = 'You';
      } else {
        // Try to find the user in contacts
        const contact = $scope.contacts.find(c => c.email === data.pinned_by || c.username === data.pinned_by);
        if (contact) {
          unpinnerName = contact.name || contact.username || data.pinned_by;
        } else {
          unpinnerName = data.pinned_by || 'Someone';
        }
      }
      
      // Find and remove the pin notification message for this message
      const notificationIndex = $scope.messages.findIndex(msg => 
        msg.type === 'pin_notification' && msg.pinned_message_id === data.message_id
      );
      
      if (notificationIndex !== -1) {
        $scope.messages.splice(notificationIndex, 1);
        console.log(`✅ Removed pin notification message from chat`);
      }
  ...
  // Handle member join notifications (new implementation)
  $scope.$on('user:joined_group', function(event, data) {
    console.log('👥 User joined group event received:', data);
    ...
    if (data && data.conversation_id && data.user) {
      // Check if this is for the currently selected group
      if ($scope.selectedContact && $scope.selectedContact.isGroup && 
          ($scope.selectedContact.id === data.conversation_id || $scope.selectedContact.conversation_id === data.conversation_id)) {
        
        // Get user display name
        const memberName = data.user.name || data.user.username || data.user.email.split('@')[0];
        
        // Create a system notification message
        const notificationMessage = {
          id: 'member_join_notification_' + data.user.email + '_' + Date.now(),
          type: 'member_join_notification',
        ...
        console.log(`✅ Added member join notification: ${memberName} joined the group`);
        ...
        ToastService.info(`${memberName} joined the group`);
        
        // Refresh group member list to include the new member
        if ($scope.selectedContact && $scope.selectedContact.isGroup) {
          $scope.loadGroupDetails($scope.selectedContact);
        }
      }
      
      // Also update the groups list to reflect new member count
      $scope.loadGroups();
    }
  });
  ...
  // Function to manually trigger member join notification (for testing or backup)
  $scope.triggerMemberJoinNotification = function(memberEmail, groupId, memberName) {
    const data = {
      group_id: groupId || ($scope.selectedContact && $scope.selectedContact.isGroup ? 
        ($scope.selectedContact.conversation_id || $scope.selectedContact.id) : null),
      member_email: memberEmail,
      member_name: memberName,
      joined_at: new Date().toISOString()
    };
    
    // Trigger the same handler as the socket event
    $scope.$broadcast('user:joined_group', {
  ...
  $scope.$on('group:member_removed', function(event, data) {
    ...
    // data = { conversation_id, member_email, member_name, left_at }
    
    // Check if this is for the currently selected group and add timeline message
    if ($scope.selectedContact && $scope.selectedContact.isGroup && 
        ($scope.selectedContact.id === data.conversation_id || $scope.selectedContact.conversation_id === data.conversation_id)) {
      
      // Get the display name of the person who left
      $scope.getPinnerNameAsync(data.member_email)
        .then(function(memberName) {
          // Create a system notification message for member leaving
          const notificationMessage = {
            id: 'member_leave_notification_' + data.member_email + '_' + Date.now(),
            type: 'member_leave_notification',
            content: `${memberName} left the group`,
            created_at: data.left_at || new Date().toISOString(),
            sender: 'system',
            sender_name: 'System',
            is_system_message: true,
          ...
          console.log(`✅ Added member leave notification: ${memberName} left the group`);
          ...
          // Show toast notification
          ToastService.info(`${memberName} left the group`);
        })
        .catch(function(error) {
          console.error('Failed to get member name for leave notification:', error);
          // Fallback with email or provided name
          const memberName = data.member_name || data.member_email.split('@')[0];
      ...
      $timeout(function() {
        ...
        if (picker && !picker._initialized) {
          ...
          picker.addEventListener('emoji-click', function(event) {
            ...
            // Create a synthetic event object to pass shift key state
            const syntheticEvent = {
              shiftKey: shiftPressed
            };
            
            $scope.$apply(function() {
              $scope.insertEmoji(emoji, syntheticEvent);
            });
          });
        }
      }, 100);
