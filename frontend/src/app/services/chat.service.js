    ...
    processMessage: function(message) {
      ...
      service.getUserInfo(message.sender).then(function(userInfo) {
        const processedMessage = {
          ...
          is_deleted: message.is_deleted, // Explicitly set both fields
          file_category: message.file_category // Preserve file_category from API
        };

        // DO NOT override type or file_category - use what the API provides
        // The backend now sends correct file_category field
        ...
        if (message.reply_to) {
          const replyPromise = service.getMessageInfo(message.reply_to).then(function(replyInfo) {
            // Detect file type for reply info too
            if (replyInfo.file_name && (replyInfo.type === 'file' || replyInfo.type === 'image')) {
              replyInfo.type = service.getFileType(replyInfo.file_name);
            }
            ...
            processedMessage.reply_info = {
              ...
              content: replyInfo.content || (replyInfo.type === 'image' ? '📷 Image' : replyInfo.type === 'video' ? '🎥 Video' : replyInfo.type === 'audio' ? '🎵 Audio' : replyInfo.type === 'file' ? '📎 File' : 'Message'),
        ...
        // Handle media messages - BUT ONLY IF NOT DELETED
        if (message.media_url && !message.content && !message.is_deleted) {
