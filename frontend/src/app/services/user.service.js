...
angular.module('vibgyorChat').factory('UserService', ['$http', '$q', 'AuthService', 'ToastService', function($http, $q, AuthService, ToastService) {
  ...
  const service = {
    ...
    unblockContact: function(email) {
      const deferred = $q.defer();

      // Same endpoint as block - it toggles the block status
      $http.post(`${API_BASE}/users/block`, { email: email }, {
        headers: service.getHeaders()
      })
        .then(function(response) {
          ToastService.success('Contact unblocked');
          deferred.resolve(response.data);
        })
        .catch(function(error) {
          ToastService.error('Failed to unblock contact');
          deferred.reject(error);
        });
    ...
    getBlockedUsers: function() {
      ...
      $http.get(`${API_BASE}/users/blocked`, {
        ...
        .then(function(response) {
          deferred.resolve(response.data.blocked_users || []);
        })
        .catch(function(error) {
    ...
    getProfilePicture: function(filename) {
      if (!filename) {
        return 'https://raw.githubusercontent.com/Shreyas-ITB/VibgyorChat/refs/heads/main/assets/static-assets-upload14076521828553999369.jpg';
      }
    ...
    loadAuthenticatedImage: function(filename) {
      ...
      $http.get(`${API_BASE}/media/profile/${filename}`, {
        ...
        .catch(function(error) {
          ...
          deferred.resolve('https://raw.githubusercontent.com/Shreyas-ITB/VibgyorChat/refs/heads/main/assets/static-assets-upload14076521828553999369.jpg');
        });
