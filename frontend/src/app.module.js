  ...
  $httpProvider.interceptors.push(['$q', '$location', '$injector', function($q, $location, $injector) {
    ...
    return {
      responseError: function(rejection) {
        ...
        // Skip interceptor for admin API calls - let admin panel handle its own errors
        if (rejection.config && rejection.config.url && rejection.config.url.includes('/admin/')) {
          console.log('Admin API call failed, letting admin panel handle the error');
          return $q.reject(rejection);
        }
        
        // Handle authentication errors (401, 403) for regular user API calls only
